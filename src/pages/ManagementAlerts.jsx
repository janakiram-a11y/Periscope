...
function AlertCard({ item, onNavigate }) {
  ...
  const { Icon } = s;
  return (
    <div style={{ background: s.bg, border: `1px solid ${s.border}`, borderRadius: 14, padding: '16px 20px', display: 'flex', gap: 14, alignItems: 'flex-start' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, marginBottom: 4 }}>
          <div style={{ fontFamily: FONT, fontSize: 13.5, fontWeight: 700, color: '#111827' }}>{item.title}</div>
        ...
        <div style={{ fontFamily: FONT, fontSize: 12, color: '#374151', lineHeight: 1.6, marginBottom: 8 }}>
          {item.description}
        </div>
...
export default function ManagementAlerts({ onNavigate }) {
  ...
  return (
    <div style={{ fontFamily: FONT, minHeight: '100%', background: '#F8FAFC' }}>
      {/* Banner */}
      <div style={{ background: 'linear-gradient(135deg, #7F1D1D 0%, #991B1B 50%, #DC2626 100%)', padding: '28px 32px 24px' }}>
        ...
        <div style={{ fontFamily: FONT, fontSize: 24, fontWeight: 800, color: 'white', marginBottom: 8 }}>Alerts Centre</div>
        <div style={{ display: 'flex', gap: 28 }}>
          {[
            ...
            <div key={i}>
              <div style={{ fontFamily: FONT, fontSize: 24, fontWeight: 800, color: k.color }}>{k.value}</div>
      ...
      <div style={{ padding: '28px 32px', maxWidth: 1100 }}>
        ...
        <div style={{ background: 'white', borderRadius: 14, boxShadow: '0 1px 4px rgba(0,0,0,.07)', overflow: 'hidden' }}>
          ...
          {tatSlaData.map((row, i) => <SlaRow key={i} row={row} i={i} />)}
        </div>
      </div>
    </div>
  );
